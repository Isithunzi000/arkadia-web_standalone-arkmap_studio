...
- **OP-1 hotfix:** self-check synchronizacji mapy czyta sumy z koperty v2 (top-level `checksums`) — po ujednoliceniu koperty w Arc 42 czytał jeszcze stary układ.
- **Higiena tożsamości (F2/F3):** `Object.freeze` na słowniku 2048 słów (+ pin runtime), odporny `_identityStore` (try/catch na otwarcie IndexedDB + `onabort`), czytelny błąd przy braku WebCrypto PBKDF2, jednorazowy feature-check WebCrypto przy otwarciu dialogu, limit 4096 znaków na wejście kodu odzyskiwania; piny pokrycia `checksums.meta` × nieznane klucze i zakres top-level.
- **Kod odzyskiwania 3–6 słów (F4):** przy tworzeniu tożsamości wybierasz długość kodu (domyślnie 3 słowa — wygodnie; do 6 — paranoia; 33–66 bitów entropii ponad nick). Nick jest kanonikalizowany (`trim` + małe litery, `^[a-z0-9]{1,32}$`) i w tej postaci zasila sól PBKDF2 — „Zbyszek" i „zbyszek" to ta sama tożsamość.
- **Podpisy Ed25519 także dla `.arkmap` (F5):** mapa i kalka podpisują się tym samym mechanizmem. Ładunek podpisu przedefiniowany: prefiks domeny + kanoniczna serializacja **całego** obiektu minus `checksums.sig` (było: sama suma pliku) — podpis dowodzi teraz integralności treści, nie tylko sumy. Przełącznik podpisów w dialogu tożsamości (domyślnie włączony); przy awarii podpisu plik zapisuje się bez podpisu z głośnym tostem (failsafe — nigdy utrata danych). Załatany wyścig T6-F2: rewizja edycji jest zrzucana **przed** asynchroniczną serializacją, więc edycja w oknie podpisu nie zgasi flagi dirty.
- **Publiczny rejestr tożsamości (F6):** rejestr nicków w publicznym repo (jeden plik `entries/<nick>.json` na nick, atomowo przez Contents API), zapisy wyłącznie przez bramkę Vercel (token wyłącznie po stronie serwera), odczyt publiczny przez CDN (spójność ostateczna — propagacja kilka minut). Tworzenie tożsamości tylko online, z dowodem posiadania klucza (PoP) i kontrolą dostępności nicku już w dialogu (✓ dostępny / ✗ zajęty / ✗ unieważniony na zawsze / ? offline). Import kodu sprawdzany w rejestrze (odmowa przy nieznanym, unieważnionym lub niezgodnym nicku oraz offline). **Unieważnienie:** wariant A — nagrobek w rejestrze (`revoked_at` + podpis właściciela), nick nigdy nie wraca do puli; dialog z potwierdzeniem wpisania nicku (duży czerwony, antyprzypadkowy). Macierz weryfikacji online dla obu formatów: zielony (zgodny) / pomarańczowy (nieznany w rejestrze) / żółty (podpis OK, tożsamość nie potwierdziła się) / czerwony (konflikt lub unieważniony) / szary (offline — bez eskalacji). Rejestr nigdy nie jest bramką wczytania.
- **Trust store TOFU (F7):** IndexedDB `arkmap-identity` v2 ze store'em `trust` — pierwszy zweryfikowany klucz danego nicka jest pinowany lokalnie; ten sam nick z **innym** kluczem to głośny toast o podszywce, działający także offline. Pliki z niezweryfikowanym podpisem nigdy nie zapisują trust; konflikt nigdy nie nadpisuje pinu.
- **Regresja:** 53 harnesse node + pełna kampania empiryczna (SMOKE + E0–E28, E15 na realnym zegarze) zielone; live E2E rejestru 9/9 na produkcji (rejestracja → odczyt publiczny → kolizje 409/403 → unieważnienie → nagrobek publiczny).

## v1.51.0 — Ujednolicona koperta formatów: `.arkmap` v2 + `.arkdelta` v3, kompaktowanie logu, proweniencja i opcjonalne podpisy Ed25519 z przenośną tożsamością (Arc 42)

Zmiana łamiąca format (świadoma, bez migracji): pliki w starych układach są odrzucane z jasnym komunikatem — zapisz je ponownie z aktualnej wersji. Wszystkie powierzchnie (zapis, wczytanie, walidacja, recenzja kalki, raporty, eksporty, specyfikacje) mówią jednym, kanonicznym kształtem koperty.

- **Koperta `.arkmap` v2 / `.arkdelta` v3 (D1/D2):** `format`, `format_version` i `checksums` na top-level w obu formatach; suma `checksums.meta` (m4) domyka integralność meta; `checksums.file` obejmuje całą treść pliku. Stare układy (m.in. `meta.checksums`, `meta.format_version`) odrzucane jako błąd fatalny — zero migracji i kompatybilności wstecznej.
- **Zachowanie nieznanych kluczy (D3):** reader zachowuje nieznane klucze 1:1 przy round-tripie (przyszłe rozszerzenia formatu nie giną w edytorze), a jednocześnie odrzuca kalki z nieznanymi polami w op/target/payload (ops-strict — zamknięte zbiory pól per typ opu; sanitery `__proto__` bez zmian, pierwszeństwo komunikatów zachowane).
- **Kompaktowanie logu kalki (D5):** 6 reguł składania (łańcuchy edycji, ADD+edycje, łańcuchy MOVE, kolory env per envId, pary ADD+DELETE znikają, scalanie batchy malowania z kolapsem per roomId) z warunkiem referencyjnym §8 (indeksy odwrotne — liniowe; wykryta i załatana regresja wstępnej wersji O(n²): buildDelta 3586 → 396 ms przy 6787 opach), idempotentne, seq przenumerowywane 1..N; niekompaktowane, poprawne kalki nadal się wczytują.
- **Proweniencja i ryzyko per-obszar (D6/D4):** meta kalki niesie proweniencję (autor, identyfikator, klucz publiczny, czas utworzenia — gdy tożsamość aktywna); panel recenzji pokazuje ryzyko każdego dotkniętego obszaru (zielony/żółty/czerwony względem sum bazy) i wyróżnia operacje niosące komendy (⚡ — special exits i user_data z kluczami komend).
- **Tożsamość autora z podpisami Ed25519 (D8, opcjonalna):** przycisk „🪪 Tożsamość autora…" nad polem wczytywania otwiera dialog: Utwórz / Importuj / Pokaż kod / Wyczyść (dwuklik). Nick jest kryptograficznie związany z kluczem — kod przeniesienia to `nick:` + sześć słów ze słownika 2048 haseł + trzyliterowa suma kontrolna (66 bitów entropii, akceptowane formy sklejone i rozdzielone, tolerancja separatorów). Z klucza wyprowadzany jest podpis kalki (Ed25519, ścieżka WebCrypto + fallback BigInt zpinned wektorami RFC 8032); kalka anonimowa nie niesie żadnych pól autora. Reader weryfikuje podpis asynchronicznie: zły podpis to głośne ostrzeżenie w panelu, nigdy odmowa wczytania. Tożsamość trzyma się w IndexedDB, identyfikator autora to 16 znaków hex z SHA-256 klucza publicznego.
- **Specyfikacje z prawdziwymi sumami (G6):** przykłady w `docs/arkmap_spec.html` §21 i `docs/arkdelta_spec.html §13` niosą realne sumy policzone silnikiem aplikacji (cross-check niezależnym oracle'm Python — zgodne bajt w bajt), przypięte harness `tests/g6_crosscheck.js` — spec i kod nie mogą się rozjechać.
- **Regresja:** 48 harnessów node + pełna kampania empiryczna (SMOKE + E0–E28, E15 na realnym zegarze) zielone; pomiar wydajności A/B na tej samej maszynie w bramce ≤5% (parse .dat +1.5%, parse .arkmap +3.1%, pipeline kalki +3.0%).

## v1.50.3 — Audyt łączny Arc 41: triage 123 findingów (5 fal, 3 silniki + sweep) — 8 realnych załatanych, 74 falsyfikacji, 38 teoretyczno-benignych (Arc 41)

Pełny audyt po v1.50.2: pięć fal findingów (renderer, I/O-async, UI/dialogi, konwertery, sweep mechaniczny) od trzech silników zewnętrznych, każdy finding zweryfikowany repro-first w kodzie i — tam, gdzie się dało — runtime w harnessie vm z pełnym bootem aplikacji. Wynik: 8 realnych (3 high, 4 low, 1 hygiene), wszystkie załatane w tej wersji; 74 falsyfikacje udokumentowane z dowodami liniowymi; 38 teoretyczno-benignych (niedojazdowe z konstrukcji). Najgłośniejsze falsyfikacje: „fitToView NaN" (zgłaszane przez 3 silniki — padding `+3`, early return na pustej mapie i clampy zoomu; potwierdzone runtime), 6× „unguarded target" w rendererze (guardy istnieją w każdym miejscu), XSS w eksporcie raportu (escaping zweryfikowany runtime), „undo kasowania gubi exits/waypointy" (przywracanie idzie przez centralny dispatcher — silniki czytały martwe lokalne closure, zob. H1), localStorage quota (8/8 `setItem` w try).

- **F1 (B-S1, high):** strukturalnie zła mapa (`areas` nie-tablica / obszar nie-obiekt / obszar bez `rooms`) przechodziła dialog walidacji jako soft-error i wywracała `applyMap` PO zatruciu stanu (`state.map` zatruty, indeksy wyzerowane, undo/redo/delta jeszcze stare). Dwie nowe warstwy obronne: ścieżki strukturalne są fatalne w `loadArkmap` (dialog bez przycisku „Wczytaj"; dopasowanie DOKŁADNE — `areas`, `areas[i]`, `areas[i].rooms`; głębsze ścieżki polowe zostają soft) plus preflight strukturalny w `applyMap` rzucający PRZED jakąkolwiek mutacją stanu (czysty rzut dla wszystkich czterech callerów, niezależnie od kliknięcia w dialogu).
- **F2 (D-S1, high):** ciche rejections loadu. `loadArkmap` nigdy już nie rejectuje z `applyMap` — choke point try/catch z tostem, `console.error` i rollbackiem `state.filename` (stara mapa nie nosi nazwy nieudanego pliku); `olLoadArkmap` awaituje load (rejection ląduje w istniejącym catch z tostem) i zwalnia `olSetBusy(false)` po fazie pobierania, trzymając `_olActiveLabel` przez czas dialogu; obie gałęzie `doLoad` (`.arkmap` i `.json→arkmap`) mają własne try/catch. Nieawaitowani callerzy (drop listener, `loadCallback`) bezpieczni z konstrukcji.
- **F3 (E-S1, high):** re-entrantny `showValDialog` — otwarcie drugiego dialogu (np. drop pliku przy otwartym dialogu walidacji) nadpisywało `_resolve` i pierwszy load wisiał na `await` na zawsze (potwierdzone runtime). Guard: nowy dialog najpierw rozwiązuje poprzedni jako `false` (anulowanie) — dokładnie to, co użytkownik widzi na ekranie. Zamyka wszystkie wektory re-entrancji w jednym miejscu.
- **F4 (low):** `readQMapSU2`/`readQMapSB`/`readQMapSC` przelączone na `_setMapKey` — klucz `__proto__` z pliku `.dat` przeżywa jako własność własna obiektu (konsumenci: `customLinesStyle`/`customLinesArrow`); wcześniej cicho ustawiał prototyp.
- **F5 (low):** live-warn importu trasy — wynik `{error:'too-many'}` z `wpDecodeRoute` nie ma pola `.valid`; guard eliminuje TypeError w listenerze `input` (warn chowa się zamiast sypać błędem w konsoli).
- **F6 (low):** `handleDropFiles` rozdziela komunikaty: upuszczenie wielu plików naraz („Wczytaj tylko jeden plik naraz") vs nieobsługiwane rozszerzenie (dotychczasowy toast formatu).
- **F7 (low):** `applyMap` zamyka menu kontekstowe — po wczytaniu nowej mapy pokazywało dane starej (akcje były oguardowane, więc skutek był czysto wizualny; typeof-guard, bo harnessy ewaluują `applyMap` bez pełnego DOM).
- **F8 (low):** `commitSetPosition` — puste/nieparsowalne pole współrzędnej daje toast błędu i przerywa zapis zamiast cichego `|| 0` (reset współrzędnej do zera).
- **H1 (hygiene):** usunięte 11 martwych closure (8× `undoFn` + 3× `doFn`; zero referencji w kodzie — undo/redo idzie wyłącznie przez centralny dispatcher, co silniki audytowe myliły z brakiem przywracania); 69 linii, zachowanie bitowo identyczne.
- **Testy:** nowy harness tests/arc41_regress.js (51 asercji: piny strukturalne kolejności/obecności/liczników + scenariusze behawioralne na snippetach wyekstrahowanych verbatim z HTML — fatalność ścieżek strukturalnych, rzuty preflightu na 6 złych strukturach, rollback filename, guard re-entrancji na 3 dialogach, `__proto__` jako własność własna, live-warn na `too-many`, isNaN guard); repin tests/raf_shim.js (123→114 call-site'ów `scheduleDraw();` — 9 usuniętych z martwych ciał H1). Pełna regresja: 44/44 harnessów node + kampania empiryczna 971 asercji (grupy SMOKE, E0–E14, E16–E28) + E15 dedykowane (15 asercji, realny cykl SW) — zero FAIL.
