...
console.log('── T9: piny ──');
{
  ...
  // piny wersji
  ok(NEW.includes("const APP_VERSION = 'v1.42.0';"), 'pin: APP_VERSION v1.42.0');
  const deltaSrc = fs.readFileSync(path.join(ROOT, 'tests', 'delta.js'), 'utf8');
  ok(deltaSrc.split('v1.42.0').length - 1 === 8, 'pin: delta.js 8x v1.42.0');
  const t2 = fs.readFileSync(path.join(ROOT, 'tests', 'tier2_state.js'), 'utf8');
  ok(t2.includes("wersja: v1.42.0"), 'pin: tier2_state.js v1.42.0');
  const t3 = fs.readFileSync(path.join(ROOT, 'tests', 'tier3_format.js'), 'utf8');
  ok(t3.includes("pin: APP_VERSION v1.42.0"), 'pin: tier3_format.js v1.42.0');
}
