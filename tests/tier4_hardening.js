...
console.log('── T9: piny ──');
{
  ...
  ok(deltaSrc.split('v1.49.5').length - 1 === 10, 'pin: delta.js 10x v1.49.5');
  const t2 = fs.readFileSync(path.join(ROOT, 'tests', 'tier2_state.js'), 'utf8');
  ok(t2.includes("wersja: v1.49.5"), 'pin: tier2_state.js v1.49.5');
  const t3 = fs.readFileSync(path.join(ROOT, 'tests', 'tier3_format.js'), 'utf8');
  ok(t3.includes("pin: APP_VERSION v1.49.5"), 'pin: tier3_format.js v1.49.5');
}
