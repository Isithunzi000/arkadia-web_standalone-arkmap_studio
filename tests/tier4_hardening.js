...
console.log('── T9: piny ──');
{
  ...
  ok(deltaSrc.split('v1.50.0').length - 1 === 9, 'pin: delta.js 9x v1.50.0 (fala 3: adnotacja integracji P3b bez numeru wersji)');
  const t2 = fs.readFileSync(path.join(ROOT, 'tests', 'tier2_state.js'), 'utf8');
  ok(t2.includes("wersja: v1.50.0"), 'pin: tier2_state.js v1.50.0');
  const t3 = fs.readFileSync(path.join(ROOT, 'tests', 'tier3_format.js'), 'utf8');
  ok(t3.includes("pin: APP_VERSION v1.50.0"), 'pin: tier3_format.js v1.50.0');
}
