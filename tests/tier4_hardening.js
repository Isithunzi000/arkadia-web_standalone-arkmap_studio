...
// ── wspolny blok stalych (jak tier3; CRC32 usuniete w v1.45.2) ──
...
console.log('── T2b: golden writera .dat (fixpoint + crc) ──');
{
  ...
  if (!fs.existsSync(FIX)) {
    ...
    // CRC-32 liczone po stronie testu (zlib) — aplikacja nie nosi juz CRC-32 (v1.45.2)
...
console.log('── T9: piny ──');
{
  ...
  // piny wersji
  ok(NEW.includes("const APP_VERSION = 'v1.45.2';"), 'pin: APP_VERSION v1.45.2');
  const deltaSrc = fs.readFileSync(path.join(ROOT, 'tests', 'delta.js'), 'utf8');
  ok(deltaSrc.split('v1.45.2').length - 1 === 10, 'pin: delta.js 10x v1.45.2');
  const t2 = fs.readFileSync(path.join(ROOT, 'tests', 'tier2_state.js'), 'utf8');
  ok(t2.includes("wersja: v1.45.2"), 'pin: tier2_state.js v1.45.2');
  const t3 = fs.readFileSync(path.join(ROOT, 'tests', 'tier3_format.js'), 'utf8');
  ok(t3.includes("pin: APP_VERSION v1.45.2"), 'pin: tier3_format.js v1.45.2');
}
