...
const VECTORS_META = JSON.parse(fs.readFileSync(path.join(__dirname, 'checksums', 'vectors_v4_meta.json'), 'utf8'));
...
const api = new Function(
  ...
  ' _encodeMetaBody, _canonBuf, _V4_DIR_ORDER, xxh3_64, xxh3_64hex };'
)();
...
console.log('── T1: golden fixture — sumy zgodne z oracle (Python/xxhash) ──');
{
  ...
  // koperta v2 (D1/D2): sumy na top-level, wpis meta zgodny z oracle
  ok(map.meta.checksums === undefined, 'meta.checksums nie istnieje (koperta v2 — top-level)');
  ok(cs.meta === VECTORS_META.golden.meta.hash, 'checksums.meta zgodne z oracle (m4)');
  ok(FIXTURE.format_version === 2, 'golden fixture: format_version 2 (koperta v2)');
}

// ═══ T1b: kodowanie meta (m4) — kotwice bajtowe i hash edge cases przeciw oracle ═══
console.log('── T1b: kodowanie meta (m4) — edge cases zgodne z oracle ──');
{
  const encMeta = (obj) => {
    const B = api._canonBuf; B.reset();
    try { api._encodeMetaBody(B, obj); return Array.from(B.bytes(), b => b.toString(16).padStart(2, '0')).join(''); }
    finally { B.release(); }
  };
  const edgeObjs = {
    empty: {},
    ...
    int_beyond_i32_as_f64: { big: 1099511627776 },
    float_noninteger: { a: 1.5 },
    float_integral_as_i32: { a: 2.0 },
    neg_zero_as_pos: { a: -0.0 },
    bools_null: { t: true, f: false, n: null },
    unicode: { s: 'zażółć gęślą jaźń 🐉' },
    nested_array: { arr: [1, 'x', null, [2]] },
    nested_object_keysorted: { o: { b: 1, a: 2, 'ą': 3 } },
    fixture_like: { name: 'golden-fixture-v4', source_file: 'golden_fixture.dat' },
  };
  for (const v of VECTORS_META.edge) {
    const obj = edgeObjs[v.name];
    ok(obj !== undefined && encMeta(obj) === v.enc_hex, 'kodowanie meta bajtowo == oracle: ' + v.name);
  }
  ok(encMeta(FIXTURE.meta) === VECTORS_META.golden.meta_enc_hex, 'kodowanie meta fixture bajtowo == oracle');
  // undefined: klucz pomijany w obiekcie, jako null w tablicy
  ok(encMeta({ a: undefined, b: 1 }) === encMeta({ b: 1 }), 'klucz z wartością undefined pomijany');
  ok(encMeta({ a: [undefined] }) === encMeta({ a: [null] }), 'undefined w tablicy == null');
  // strażnik głębokości: > 60 poziomów → wyjątek (w verify → verifyError)
  let deep = {}, cur = deep;
  for (let i = 0; i < 70; i++) { cur.n = {}; cur = cur.n; }
  let threw = false;
  try { encMeta(deep); } catch (e) { threw = true; }
  ok(threw, 'zagnieżdżenie > 60 → kontrolowany wyjątek (meta-canon-depth)');
}
...
console.log('── T3: kanonizacja (strip-equivalence, shuffled keys, pole wewnetrzne area) ──');
{
  ...
  const hBase = base.checksums.rooms['5'];
  ...
  ok(explicit.checksums.rooms['5'] === hBase,
     'jawne domyślne + puste kontenery + przestawione klucze + pole area → ta sama suma');
  ok(explicit.checksums.file === base.checksums.file,
...
console.log('── T4: verifyChecksums — OK / korupcja pokoju / obszaru / colors / brak wpisu ──');
{
  ...
  ok(r0.computed && r0.computed.alg === 'v4' && r0.computed.file === map.checksums.file,
  ...
  delete mMiss.checksums.rooms['3'];
...
console.log('── T4c: missingAreas / extraRooms / extraAreas → ok:false ──');
{
  ...
  delete mMissA.checksums.areas['-5'];
  ...
  mXtraR.checksums.rooms['777'] = 'deadbeefdeadbeef';
  ...
  mXtraA.checksums.areas['888'] = 'deadbeefdeadbeef';
  ...
  mSort.checksums.rooms['10'] = 'a'.repeat(16);
  mSort.checksums.rooms['9'] = 'b'.repeat(16);
...
console.log('── T5: algMismatch głośny (v1/v2/v3/v9), brak sum cichy ──');
{
  ...
  const mV1 = freshMap();
  mV1.checksums = { file: 'deadbeef', areas: {}, rooms: {} };   // v1: bez pola alg
  ...
  const mV3 = freshMap();
  mV3.checksums = { alg: 'v3', file: 'deadbeefdeadbeef', areas: {}, rooms: {} };
  ...
  const mV9 = freshMap();
  mV9.checksums = { alg: 'v9', file: 'deadbeefdeadbeef', areas: {}, rooms: {} };
  ...
  // koperta v2 (D1): stara lokalizacja meta.checksums jest IGNOROWANA (nie ma cichego fallbacku)
  const mLegacy = freshMap();
  mLegacy.meta.checksums = { alg: 'v4', file: 'deadbeefdeadbeef', areas: {}, rooms: {} };
  const r4 = api.verifyChecksums(mLegacy);
  ok(r4.present === false && r4.ok === true,
     'sumy wyłącznie w meta.checksums (układ v1) → present:false (ignorowane)');
}

// ═══ T5c: checksums.meta (D2) — integrity meta informacyjnie, poza ok ═══
console.log('── T5c: checksums.meta — metaOk informacyjny; identity (file) odporna na edycje meta ──');
{
  const mOk = freshMap(); api.addChecksums(mOk);
  const r0 = api.verifyChecksums(mOk);
  ok(r0.metaOk === true, 'świeży plik: metaOk === true');

  const mMeta = freshMap(); api.addChecksums(mMeta);
  mMeta.meta.source_file = 'podmienione.dat';   // edycja meta PO zapisie sum
  const r1 = api.verifyChecksums(mMeta);
  ok(r1.metaOk === false && r1.ok === true && r1.fileOk === true,
     'edycja meta → metaOk:false, ale ok/fileOk bez zmian (identity ≠ integrity)');

  const mNoMeta = freshMap(); api.addChecksums(mNoMeta);
  delete mNoMeta.checksums.meta;
  const r2 = api.verifyChecksums(mNoMeta);
  ok(r2.metaOk === undefined && r2.ok === true,
     'brak wpisu checksums.meta → metaOk:undefined, ok bez zmian');

  // app_version objęte checksums.meta (D1): stempel tylko gdy APP_VERSION zdefiniowane
...
console.log('── T6: piny strukturalne ──');
{
  ...
  ok(HTML.includes("const APP_VERSION = 'v1.52.6';"), 'pin: APP_VERSION v1.52.6');
}
...
console.log('── T7: pin NaN-kanoniczny (NaN/undefined w CL → identyczne bajty) ──');
{
  ...
  ok(mN.checksums.rooms['900'] === mU.checksums.rooms['900'],
