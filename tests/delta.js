...
console.log('— T2: buildDelta — determinizm i kształt pliku —');
{
  ...
  ok(d.meta.format === 'arkdelta' && d.meta.format_version === 2, 'meta: format + format_version 2 (v1.49.5)');
...
ok(HTML.includes('state.baseInfo = _computeBaseInfo(null, state._pendingComputed);'),
  'integracja: baseInfo w wrapperze applyMap z reużyciem computed z verify (v1.49.5)');
...
ok(HTML.includes("const APP_VERSION = 'v1.49.5';"), 'wersja: v1.49.5');
...
}
{
  ...
  ok(HTML.includes("const APP_VERSION = 'v1.49.5';"), 'wersja v1.49.5 w HTML');
}
{
