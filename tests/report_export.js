...
// panel recenzji kalki (dp-*). Wspolny builder: buildDiagnosticsReport.
...
console.log('── T1: piny strukturalne (builder, DOM, wiring) ──');
{
  ...
  ok(/function _valReportSections\(valRes, chkRes\)/.test(HTML), 'sekcje raportu walidacji pliku');
...
console.log('── T2: buildDiagnosticsReport (funkcjonalnie) ──');
{
  ...
  const api = new Function('APP_VERSION', code)('v1.45.1');
  ...
  ok(r1.includes('- Wersja aplikacji: v1.45.1'), 'naglowek: wersja aplikacji');
...
{
  const code = extract(HTML, 'function _valReportSections(valRes, chkRes) {') + '\nreturn { _valReportSections };';
  ...
  ok(secs.length === 3, '3 sekcje: bledy / ostrzezenia / sumy');
...
console.log('── T4: _deltaReviewReportText (funkcjonalnie) ──');
{
  ...
  const api = new Function('APP_VERSION', 'state', code)('v1.45.1', state);
...
console.log('── T5: regla kompletnosci powierzchni ──');
{
  ...
  // trzy powierzchnie z raportem: val-modal, vd-*, dp-*
  ok(HTML.includes('id="val-btn-copy"') && /vd-copy|vdCopyReport/.test(HTML) && HTML.includes('id="dp-copy-report"'),
     'wszystkie 3 powierzchnie diagnostyczne maja eksport do schowka');
}
