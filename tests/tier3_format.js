...
{
  ok(NEW.includes("const APP_VERSION = 'v1.52.0';"), 'pin: APP_VERSION v1.52.0');
  const deltaSrc = fs.readFileSync(path.join(ROOT, 'tests', 'delta.js'), 'utf8');
  ok((deltaSrc.match(/v1\.52\.0/g) || []).length === 8 && (deltaSrc.match(/v1\.51\.0/g) || []).length === 1, 'pin: delta.js 8x v1.52.0 (piny APP_VERSION po bumpie) + adnotacja formatu v1.51.0');
