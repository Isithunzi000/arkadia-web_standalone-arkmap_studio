...
// ═══ T7: strazniki strukturalne + piny v1.38.0 ═══
console.log('── T7: strazniki strukturalne Tier 3 + piny wersji ──');
{
  ok(NEW.includes("const APP_VERSION = 'v1.38.0';"), 'pin: APP_VERSION v1.38.0');
  const deltaSrc = fs.readFileSync(path.join(ROOT, 'tests', 'delta.js'), 'utf8');
  ok((deltaSrc.match(/v1\.38\.0/g) || []).length === 8, 'pin: delta.js 8x v1.38.0 (4 linie x includes+label)');
