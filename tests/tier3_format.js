// Harness Tier 3 (v1.48.1) — sumy v4 XXH3-64 (W3, implementacja Arc 19, migracja Arc 20),
...
// usuniete w v1.48.1 — kalka na XXH3-64)
...
console.log('── T2 (W3): addChecksums/verifyChecksums — v4, badAreas, missingRooms, algMismatch ──');
{
  ...
  // Arc 20 (v1.48.1): kazdy alg != v4 to GLOSNY algMismatch (present:true, ok:false) —
...
{
  ok(NEW.includes("const APP_VERSION = 'v1.48.1';"), 'pin: APP_VERSION v1.48.1');
  const deltaSrc = fs.readFileSync(path.join(ROOT, 'tests', 'delta.js'), 'utf8');
  ok((deltaSrc.match(/v1\.48\.1/g) || []).length === 10, 'pin: delta.js 10x v1.48.1 (4 linie x includes+label + 2 komentarze Arc 20)');
