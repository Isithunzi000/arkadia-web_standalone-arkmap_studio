...
function benchFile(file) {
  const raw = fs.readFileSync(file);
  const parse = [], stream = [];
  let heap = 0, rooms = 0, roomsStream = 0;
  const WARM = 3;
  for (let i = 0; i < WARM + N; i++) {
    ...
    if (i >= WARM) { parse.push(t1 - t0); heap = process.memoryUsage().heapUsed - h0; rooms = n; }
  }
  for (let i = 0; i < WARM + N; i++) {
    global.gc();
    const t0 = performance.now();
