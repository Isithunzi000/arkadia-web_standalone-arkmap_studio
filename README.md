...
- **Własny format `.arkmap`** — czytelny JSON przyjazny gitowi (stabilna serializacja, sumy kontrolne XXH3-64, opcjonalny podpis autora Ed25519). Format obejmuje też uniwersalne **linie transportów** (`transports`, standard `arkmap-transports` v1) z sumami kontrolnymi per linia (`checksums.transports`) — wykrywają, którą linię zmieniono, dodano albo usunięto.
- **Kalka zmian `.arkdelta`** — zapisuj same swoje edycje (z sesji albo z różnicy dwóch map) i nanieś je na nowszą wersję mapy upstream przez recenzję z wykrywaniem konfliktów — bez wysyłania całej mapy. Kalka może być anonimowa albo podpisana tożsamością autora.
- **Tożsamość autora (opcjonalna)** — podpisy Ed25519 map i kalek: nick kryptograficznie związany z kluczem, kod przeniesienia z 3–6 słów, publiczny rejestr nicków (jeden nick = jeden klucz, trwałe unieważnienie przez właściciela) i lokalny pinning kluczy z alarmem podszywki działającym także offline. Klucz prywatny nigdy nie opuszcza przeglądarki.
...
- [Mega-test silników map — 2026-08-30](https://isithunzi000.github.io/arkadia-web_standalone-arkmap_studio/docs/megatest_raport_2026-08-30.html) — powtórka drabinki 27k–432k na tej samej maszynie i w jednej sesji (aplikacja v1.52.5, seed i pary identyczne). Wyróżnik: Mudlet desktop 5.0.0 (poprzedni przebieg: 4.22.0), więc run wyszedł niezależnym benchmarkiem wydania — `loadMap` 3,2–3,6× szybciej (432k: 31,9 → 10,1 s), `getPath` 3,3–5,7× szybciej, `searchRoom` ~3,6× szybciej, kosztem +9% RAM peak (1598 → 1741 MB); `.arkmap` szybszy od `.dat` na całej drabince (ratio 0,45–0,86); heap UI przy 432k: `.arkmap` 124 MB vs `.dat` 489 MB; gate kosztowy zielony, found zgodne z desktopem na każdym szczeblu ([`tests/megatest/`](tests/megatest/))
- [Mega-test silników map — 2026-08-28](https://isithunzi000.github.io/arkadia-web_standalone-arkmap_studio/docs/megatest_raport_2026-08-28.html) — jedna drabinka (27k–432k pokoi): silniki (ArkMap Node, czytnik mudlet-web, Mudlet desktop) + natywne apki w headless Chromium (prawdziwe UI ArkMap Studio vs oryginalny pipeline npm `mudlet-map-renderer`, Mudlet desktop AppImage); wczytanie + pathfinding + przeszukanie + iteracja + pamięć; deterministyczne 100 par pokoi i 3 frazy (seed), 5 przebiegów, jedna maszyna i jedna sesja. Wyróżniki: pełna zgodność found z desktopem na każdym szczeblu; podwójny pomiar wczytania (load + verified — weryfikacja sum kontrolnych dzieje się w tle po pierwszej klatce i nie blokuje UI); gate kosztowy zielony na 101 parach (nasz A\* zawsze równy kosztowo naszemu Dijkstrze, ich A\* nigdy tańszy poza trasami przez locki, które ich silnik ignoruje); A\* ArkMap najszybszy od ~54k pokoi w górę (432k: 248 ms vs 1733 ms ich A\*; na 27k ich A\* minimalnie szybszy: 596 vs 768 ms); szukajka z indeksem tokenowym: 15 ms na 432k pokoi; ujawniony natywny liAGPL-3.0-only stacku mudlet-web — powyżej 50k pokoi czytnik przechodzi w tryb skeleton z pustym `getRooms()`, więc ich pathfinding i iteracja są tam niedostępne ([`tests/megatest/`](tests/megatest/))
- **Pakiet npm [`arkmap`](https://github.com/Isithunzi000/arkmap)** — biblioteka (Node ≥ 18 i przeglądarki, zero zależności) z tą samą logiką co Studio: I/O `.dat` ↔ `.arkmap`, walidacja, sumy kontrolne, routing (A*/Dijkstra, filtry kierunków, transporty, locki), waypointy (kody `ARKMAP2:`), diff map (`arkmap/diff`) i dane transportów Arkadii. [Demo viewer online](https://isithunzi000.github.io/arkmap/)
...
[AGPL-3.0-only](LICENSE) · atrybucje zasobów zewnętrznych: [NOTICE](NOTICE.md)
